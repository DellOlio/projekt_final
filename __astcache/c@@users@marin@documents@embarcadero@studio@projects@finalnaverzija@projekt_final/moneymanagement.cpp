...
void __fastcall TMoneyManagementForm::LoadNotesButtonClick(TObject *Sender)
{

	if (OpenDialogText->Execute())
	{
			DataModule1 -> KorisnikTable ->Edit();
			 static_cast<TBlobField*>(DataModule1->KorisnikTable->FieldByName("Bilješke"))->LoadFromFile(OpenDialogText->FileName);
			DataModule1 -> KorisnikTable->Post();
...
void __fastcall TMoneyManagementForm::SaveNotesButtonClick(TObject *Sender)
{

	if (SaveDialog1->Execute()) {

		static_cast<TBlobField*>(DataModule1->KorisnikTable->FieldByName("Bilješke"))->SaveToFile(SaveDialog1->FileName);
